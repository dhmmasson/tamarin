...
const experiencePlan = generatePlan( { "tau": {} }, {
coefficients:
  { random   : [ 0 ]
  , original : [ 1 ] }
, blur  : [ 20 ]
    ...
    if( values instanceof Array ) {
      for( const val of values ) {
        for( const testName in results ) {
          newResult[ testName + "_" + name + ":" + val ] = { ...results[ testName ] } ;
...
function test( nbClassement, nbRepetition, nbAlternative, planEx ) {
  ...
  const vars = { original : "ordinal"
  , random   : "ordinal" } ;
  const results = [] ;

  // DO as many ordering as required
  for( let classementIndex = 0 ; classementIndex < nbClassement ; classementIndex++ ) {
    ...
    for( let repetitionIndex = 0 ; repetitionIndex < nbRepetition ; repetitionIndex++ ) {
      ...
      if( maxDistance === 0 ) {
        ...
        for( const experience in experiencePlan ) {
          const kendalls = computeKendallTau( distribution, planEx[ experience ], maxDistance ) ;
...
function init( ) {
  ...
  for( const varname in vars ) {
    ...
    const experience = experiencePlan[ varname.slice( 0, varname.lastIndexOf( "_" ) ) ] ;

    console.log( varname, experience, mean, standardDeviation ) ;
    if( experience ) statsReport.push( [ experience.coefficients.original, experience.coefficients.random, experience.blur, mean, standardDeviation ] ) ;
  }
  ...
  for( const i in results ) {
    ...
    for( const name in line ) {
      ...
      if( name !== "maxDistance" ) {
        ...
        const header = name.split( "_" ).map( e => e.split( ":" ) ).reduce( ( a, e ) => { if( e[ 1 ] ) [ , a[ e[ 0 ] ] ] = e ; return a ; }, {} ) ;
...
function normalizedTauB( stats, reference, maxDistance ) { return ( 1 - stats.kendallsTau( reference, "baboonOrder" ).b.tauB ) / 2 / maxDistance ; }

function computeKendallTau( values, experience, maxDistance ) {
  const vars = { original    : "ordinal"
  , random      : "ordinal"
  , baboonOrder : "ordinal" } ;
  const data = computeBaboon( values, experience.shape, experience.coefficients, experience.blur ) ;
  console.table( data.sort( ( a, b ) => a.random - b.random ) ) ;
  const stats = ( new Statistics( data, vars ) ) ;
  return { originalD : normalizedTauB( stats, "original", maxDistance )
...
function computeBaboon( values, shape, weight, blur ) {
  return recomputeOrder( values, shape, blur )
    .map( e => { ( e.baboon = e.original / listLength * weight.original + e.normalize * weight.random ) ; return e ; } )
    .sort( ( a, b ) => ( a.baboon - b.baboon ) )
    .map( ( e, i, arr ) => { e.baboonOrder = ( e.baboon === ( arr[ i - 1 ] || {} ).baboon ) ? arr[ i - 1 ].baboonOrder : i ; return e ; } ) ;
}
...
function recomputeOrder( values, shape, blur ) {
  ...
  const array = values
    ...
    .map( ( e, i, arr ) => { e.normalize = e.dominance / arr[ arr.length - 1 ].dominance ; return e ; } )
    .map( ( e, i, arr ) => { e.rank = ( e.dominance === ( arr[ i - 1 ] || {} ).dominance ) ? arr[ i - 1 ].rank : i ; return e ; } ) ;
 ...
 * @return {}   description
 */
const generateSeed = alternativeCount => Array( alternativeCount )
  .fill( 1 )
...
function generateData( alternatives ) {
  const baseArray = ( alternatives instanceof Array ) ? alternatives.slice() : generateSeed( alternatives ) ;
  return baseArray.sort( ( a, b ) => ( a.seed - b.seed ) )
    .map( ( a, i ) => ( { ...a
    , value  :
...
function computeDistribution( n ) {
  ...
  Weights.binomial = Array( n ).fill( 1 ).map( ( e, i ) => Math.pow( ( i - n / 2 ), 2 ) ) ;
  Weights.sinus = Array( n ).fill( 1 ).map( ( e, i ) => Math.sin( i / n * Math.PI ) ) ;
  // compute total
  const Total = {} ;
  ...
  // computeCenter
  const Center = {} ;
  ( Center.uniform = Size.uniform.slice() ).reduce( ( acc, e, i, arr ) => { const size = arr[ i ] ; arr[ i ] = acc + size / 2 ; return acc + size ; }, 0 ) ;
  ( Center.binomial = Size.binomial.slice() ).reduce( ( acc, e, i, arr ) => { const size = arr[ i ] ; arr[ i ] = acc + size / 2 ; return acc + size ; }, 0 ) ;
  ( Center.sinus = Size.sinus.slice() ).reduce( ( acc, e, i, arr ) => { const size = arr[ i ] ; arr[ i ] = acc + size / 2 ; return acc + size ; }, 0 ) ;

  const randomize = ( cellCenters, cellSizes ) => n => cellCenters[ n ] + ( Math.random() - 0.5 ) * cellSizes[ n ] ;
  return { uniform  : randomize( Center.uniform, Size.uniform )
