...
class Connector {
   ...
   */
  importSql() {
    const storeQuery = (filename) => (query) => {
        this.sql[basename(filename, ".sql")] = query;
      },
      getContent = (filename) =>
        fs.readFile(resolve(this.sqlRoot, filename), "utf8"),
      readSqlFile = (filename) =>
        getContent(filename).then(storeQuery(filename)),
      readFiles = (files) => Promise.all(files.map(readSqlFile));

    return fs.readdir(this.sqlRoot).then(readFiles).catch(console.warn);
  }
   ...
   */
  getCriteria() {
    return this.pool.query(this.sql.criteria_get_all).then(cleanCriterion);
  }
  ...
  getTechnologies() {
    // TODO: join with technology table to gather the description
    return this.pool.query(this.sql.data_get_all).then(pivotEvaluation);
  }
}
